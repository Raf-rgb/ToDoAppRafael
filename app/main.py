...
def edit_task(task: Task):
    ...
    description = st.text_area("Descripcion", value=task.get("description", ""), key="task_desc")
    ...
    for task in tasks:
        ...
        with st.expander(task.get("title", ""), expanded=False):
            ...
            if status == TaskStatus.PENDING or status == TaskStatus.IN_PROGRESS:
                ...
                with col_done_btn:
                    if st.button("Done", 
                                 type="primary" if status == TaskStatus.IN_PROGRESS else "secondary", 
                ...
                if status == TaskStatus.PENDING:
                    if st.button(
                        "Start", 
                        type="primary", 
                        use_container_width=True, key=f"btn_start_{task_id}"):
                        task["status"] = TaskStatus.IN_PROGRESS.value
                        update_task(task)

            else:
                if st.button("Delete", icon="🗑️", type="primary", key=f"btn_delete_{task_id}"):
                    delete_task(task)
